...
export default{
    ...
    "nonLeafChild": [
        ...
        },
        {
            ...
            "leafChild": [
                ...
                },
                {
                    ...
                    "summary": "",
                    "src": "/split-array-by-prime-indices/",
                    "original_src": "https://leetcode.cn/problems/split-array-by-prime-indices/",
                    "sort": 0,
                    "isLeaf": true,
                    "solution": null,
                    "score": 1227.21323323,
            ...
            "nonLeafChild": [
                {
                    "title": "§1.3 质因数分解",
                    "summary": "模板一，预处理每个数的所有不同质因子。原理同埃氏筛。<br>```py [sol-Python3]<br>MX = 1_000_001<br>prime_factors = [[] for _ in range(MX)]<br>for i in range(2, MX):<br>if not prime_factors[i]:  # i 是质数<br>for j in range(i, MX, i):  # i 的倍数 j 有质因子 i<br>prime_factors[j].append(i)<br>```<br>```java [sol-Java]<br>class Solution {<br>private static final int MX = 1_000_001;<br>private static final List<Integer>[] primeFactors = new ArrayList[MX];<br>private static boolean initialized = false;<br>// 这样写比 static block 更快<br>private void init() {<br>if (initialized) {<br>return;<br>}<br>initialized = true;<br>Arrays.setAll(primeFactors, _ -> new ArrayList<>());<br>for (int i = 2; i < MX; i++) {<br>if (primeFactors[i].isEmpty()) { // i 是质数<br>for (int j = i; j < MX; j += i) { // i 的倍数 j 有质因子 i<br>primeFactors[j].add(i);<br>}<br>}<br>}<br>}<br>public int solve(int[] nums) {<br>init(); // 记得初始化<br>}<br>}<br>```<br>```cpp [sol-C++]<br>const int MX = 1'000'001;<br>vector<int> prime_factors[MX];<br>int init = [] {<br>for (int i = 2; i < MX; i++) {<br>if (prime_factors[i].empty()) { // i 是质数<br>for (int j = i; j < MX; j += i) { // i 的倍数 j 有质因子 i<br>prime_factors[j].push_back(i);<br>}<br>}<br>}<br>return 0;<br>}();<br>```<br>```go [sol-Go]<br>const mx = 1_000_001<br>var primeFactors = [mx][]int{}<br>func init() {<br>for i := 2; i < mx; i++ {<br>if primeFactors[i] == nil { // i 是质数<br>for j := i; j < mx; j += i { // i 的倍数 j 有质因子 i<br>primeFactors[j] = append(primeFactors[j], i)<br>}<br>}<br>}<br>}<br>```<br>模板二，预处理 $x$ 的最小质因子 $\\text{LPF}(x)$，从而做到 $\\mathcal{O}(\\log x)$ 分解 $x$。可以求出 $x$ 的每个质因子的个数。<br>```py [sol-Python3]<br>MX = 1_000_001<br>lpf = [0] * MX<br>for i in range(2, MX):<br>if lpf[i] == 0:  # i 是质数<br>for j in range(i, MX, i):<br>if lpf[j] == 0:  # 首次访问 j<br>lpf[j] = i<br>",
                    "src": "",
                    "original_src": "",
                    "sort": 0,
                    "isLeaf": false,
        ...
        {
            "title": "例如 prime_factorization(45) = [(3, 2), (5, 1)]，表示 45 = 3**2 * 5**1",
            "summary": "",
            "src": "",
            "original_src": "",
            "sort": 0,
        ...
        },
        {
            ...
            "nonLeafChild": [
                ...
                {
                    "title": "§1.5 因子",
                    "summary": "模板（预处理每个数的所有因子）：<br>```py [sol-Python3]<br>",
                    "src": "",
                    "original_src": "",
                    "sort": 0,
                    "isLeaf": false,
        ...
        {
            "title": "预处理每个数的因子",
            "summary": "MX = 1_000_001  # **根据题目调整**<br>divisors = [[] for _ in range(MX)]<br>for i in range(1, MX):<br>for j in range(i, MX, i):  # 枚举 i 的倍数 j<br>divisors[j].append(i)  # i 是 j 的因子<br>```<br>```java [sol-Java]<br>class Solution {<br>private static final int MX = 1_000_001; // **根据题目调整**<br>private static final List<Integer>[] divisors = new ArrayList[MX];<br>private static boolean initialized = false;<br>// 这样写比 static block 更快<br>private void init() {<br>if (initialized) {<br>return;<br>}<br>initialized = true;<br>Arrays.setAll(divisors, _ -> new ArrayList<>());<br>for (int i = 1; i < MX; i++) {<br>for (int j = i; j < MX; j += i) { // 枚举 i 的倍数 j<br>divisors[j].add(i); // i 是 j 的因子<br>}<br>}<br>}<br>public int solve(int[] nums) {<br>init(); // 记得初始化<br>}<br>}<br>```<br>```cpp [sol-C++]<br>const int MX = 1'000'001; // **根据题目调整**<br>vector<int> divisors[MX];<br>int init = [] {<br>for (int i = 1; i < MX; i++) {<br>for (int j = i; j < MX; j += i) { // 枚举 i 的倍数 j<br>divisors[j].push_back(i); // i 是 j 的因子<br>}<br>}<br>return 0;<br>}();<br>```<br>```go [sol-Go]<br>const mx = 1_000_001 // **根据题目调整**<br>var divisors [mx][]int // 如果 mx 很大可能会 MLE，可以改成 int32<br>func init() {<br>for i := 1; i < mx; i++ {<br>for j := i; j < mx; j += i { // 枚举 i 的倍数 j<br>divisors[j] = append(divisors[j], i) // i 是 j 的因子<br>}<br>}<br>}<br>```<br>",
            "src": "",
            "original_src": "",
            "sort": 0,
            "isLeaf": false,
            ...
            "nonLeafChild": [
                {
                    ...
                    "leafChild": [
                        ...
                        },
                        {
                            "title": "3671. 子序列美丽值求和",
                            "summary": "",
                            "src": "/sum-of-beautiful-subsequences/",
                            "original_src": "https://leetcode.cn/problems/sum-of-beautiful-subsequences/",
                            "sort": 0,
                            "isLeaf": true,
                            "solution": null,
                            "score": 2647.1423924149,
                            "leafChild": [],
                            "nonLeafChild": [],
                            "isPremium": false,
                            "last_update": ""
                ...
                {
                    "title": "二、组合数学",
                    "summary": "快速幂、组合数的预处理**模板**见 <a href=\"https://leetcode.cn/circle/discuss/mDfnkW/\">模运算的世界：当加减乘除遇上取模</a><br>",
                    "src": "",
                    "original_src": "",
                    "sort": 0,
                    "isLeaf": false,
                    ...
                    "nonLeafChild": [
                        ...
                        {
                            "title": "§2.3 放球问题",
                            "summary": "<a href=\"https://leetcode.cn/problems/count-ways-to-make-array-with-product/solutions/2713481/tu-jie-zhi-yin-zi-fen-jie-fang-qiu-wen-t-fboo/\">图解：多重集组合数</a><br>**思维扩展**：<br>",
                            "src": "",
                            "original_src": "",
                            "sort": 0,
                            "isLeaf": false,
                ...
                },
                {
                    ...
                    "nonLeafChild": [
                        ...
                        },
                        {
                            ...
                            "leafChild": [
                                ...
                                },
                                {
                                    ...
                                    "summary": "",
                                    "src": "/count-number-of-trapezoids-ii/",
                                    "original_src": "https://leetcode.cn/problems/count-number-of-trapezoids-ii/",
                                    "sort": 0,
                                    "isLeaf": true,
                                    "solution": null,
                                    "score": 2643.4108112417,
                                    "leafChild": [],
                                    "nonLeafChild": [],
                                    "isPremium": false,
                                    "last_update": ""
                        ...
                        {
                            "title": "§5.4 凸包",
                            "summary": "另见 <a href=\"/list/dynamic_programming\">动态规划题单</a> 的「**§11.7 斜率优化 DP**」。<br>",
                            "src": "",
                            "original_src": "",
                            "sort": 0,
                            "isLeaf": false,
                ...
                },
                {
                    ...
                    "nonLeafChild": [
                        {
                            "title": "处理 x 的逻辑写下面",
                            "summary": "从小到大枚举回文数的模板（从 $1$ 开始枚举）：<br>```py [sol-Python3]<br>def gen_palindrome() -> Iterator[int]:<br>base = 1<br>while True:<br>for i in range(base, base * 10):<br>s = str(i)<br>x = int(s + s[::-1][1:])<br>yield x<br>for i in range(base, base * 10):<br>s = str(i)<br>x = int(s + s[::-1])<br>yield x<br>base *= 10<br>for x in gen_palindrome():<br>if x > 10 ** 9:  # 根据题目调整<br>break<br>print(x)<br>```<br>```java [sol-Java]<br>class Solution {<br>private static final long MX = 10_000_000_000L; // 根据题目调整<br>private static final List<Long> palindromes = new ArrayList<>();<br>private static boolean initialized = false;<br>// 这样写比 static block 快<br>private void init() {<br>if (initialized) {<br>return;<br>}<br>initialized = true;<br>for (int base = 1; ; base *= 10) {<br>// 生成奇数长度回文数，例如 base = 10，生成的范围是 101 ~ 999<br>for (int i = base; i < base * 10; i++) {<br>long x = i;<br>for (int t = i / 10; t > 0; t /= 10) {<br>x = x * 10 + t % 10;<br>}<br>if (x > MX) {<br>return;<br>}<br>palindromes.add(x);<br>}<br>// 生成偶数长度回文数，例如 base = 10，生成的范围是 1001 ~ 9999<br>for (int i = base; i < base * 10; i++) {<br>long x = i;<br>for (int t = i; t > 0; t /= 10) {<br>x = x * 10 + t % 10;<br>}<br>if (x > MX) {<br>return;<br>}<br>palindromes.add(x);<br>}<br>}<br>}<br>public int solve(int n) {<br>init(); // 记得调用<br>}<br>}<br>```<br>```cpp [sol-C++]<br>const long long MX = 10'000'000'000LL; // 根据题目调整<br>vector<long long> palindromes;<br>// 预处理 [1,MX] 中的回文数<br>auto init = []() {<br>for (int base = 1; ; base *= 10) {<br>// 生成奇数长度回文数，例如 base = 10，生成的范围是 101 ~ 999<br>for (int i = base; i < base * 10; i++) {<br>long long x = i;<br>for (int t = i / 10; t > 0; t /= 10) {<br>x = x * 10 + t % 10;<br>}<br>if (x > MX) {<br>return 0;<br>}<br>palindromes.push_back(x);<br>}<br>// 生成偶数长度回文数，例如 base = 10，生成的范围是 1001 ~ 9999<br>for (int i = base; i < base * 10; i++) {<br>long long x = i;<br>for (int t = i; t > 0; t /= 10) {<br>x = x * 10 + t % 10;<br>}<br>if (x > MX) {<br>return 0;<br>}<br>palindromes.push_back(x);<br>}<br>}<br>}();<br>```<br>```go [sol-Go]<br>const mx = 10_000_000_000 // 根据题目调整<br>var palindromes []int<br>// 预处理 [1,mx] 中的回文数<br>func init() {<br>for base := 1; ; base *= 10 {<br>// 生成奇数长度回文数，例如 base = 10，生成的范围是 101 ~ 999<br>for i := base; i < base*10; i++ {<br>x := i<br>for t := i / 10; t > 0; t /= 10 {<br>x = x*10 + t%10<br>}<br>if x > mx {<br>return<br>}<br>palindromes = append(palindromes, x)<br>}<br>// 生成偶数长度回文数，例如 base = 10，生成的范围是 1001 ~ 9999<br>for i := base; i < base*10; i++ {<br>x := i<br>for t := i; t > 0; t /= 10 {<br>x = x*10 + t%10<br>}<br>if x > mx {<br>return<br>}<br>palindromes = append(palindromes, x)<br>}<br>}<br>}<br>```<br>",
                            "src": "",
                            "original_src": "",
                            "sort": 0,
                            "isLeaf": false,
                        ...
                        {
                            "title": "§7.6 线性基",
                            "summary": "见 <a href=\"/list/bitwise_operations\">位运算题单</a>。<br>",
                            "src": "",
                            "original_src": "",
                            "sort": 0,
                            "isLeaf": false,
